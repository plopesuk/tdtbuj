...
contains
  ...
  function vcg(mp,l,m,n,sol)
    ...
    summ=0.0_k_pr
    do i=0,int(mp/2)
      summ=summ+(-1.0_k_pr)**i*sol%fact(mp)/(sol%fact(2*i)*sol%fact(mp-2*i))*&
          (m/sqrt(1.0_k_pr-n*n))**(2*i)*(l/sqrt(1.0_k_pr-n*n))**(mp-2*i)
    enddo
    vcg=summ
  end function vcg
...
!       integer ::t
    ...
    case (2)
      ...
      t2=0.0_k_pr
      if (abs(mp-1)<=l) &
        t1=sqrt(real((l+mp)*(l-mp+1),k_pr))*rotmmpl(m,mp-1,l,n,sol)
      if (abs(mp+1)<=l) &
        t2=-sqrt(real((l-mp)*(l+mp+1),k_pr))*rotmmpl(m,mp+1,l,n,sol)
  ...
  function SmmplP(alpha,m,mp,l,n,cg,sg,sol)
    ...
    select case(alpha)
    case (1)
      SmmplP=absm*bm(cg,sg,m)*(((-1)**mp)*rotmmpl(absm,mp,l,n,sol)&
        +rotmmpl(absm,-mp,l,n,sol))
    case (2)
      SmmplP=am(cg,sg,m)*(((-1)**mp)*RotmmplP(alpha,absm,mp,l,n,sol)&
        +RotmmplP(alpha,absm,-mp,l,n,sol))
    case(3)
      SmmplP=0.0_k_pr
    end select
