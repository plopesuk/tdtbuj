...
!> \internal add the science citation + document parameters
  subroutine SimulAnnealing (n, x, Max, rt, eps, ns, nt, neps, maxevl, lb, ub, c, t, vm, xopt, fopt, nacc, nfcnev, nobds, ier, &
 & fstar, xp, nacp, func, gen, atomic, sol, tb, io)
    character (len=*), parameter :: myname = "SimulAnnealing"
    real (k_pr) :: x (:), c (:), vm (:), lb (:), ub (:), fstar (:), xopt (:), xp (:), t, eps, rt, fopt
    integer :: nacp (:), n, ns, nt, neps, nacc, maxevl, nobds, ier, nfcnev
    logical :: Max
...
!  set initial values.
    ...
    do i = 1, n
      if ((x(i) < lb(i)) .or. (x(i) > ub(i))) then
        write (saux, '(i0)') i
        call error ("the starting value of parameter "//trim(saux)//" is outside the bounds execution terminated "//"without any op&
       &timization. lb(i) < x(i) < ub(i), i = 1, n.", myname, .false., io)
        ier = 2
        return
      end if
    end do
