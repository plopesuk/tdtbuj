...
subroutine computeB(cxz)
  ...
  real(k_pr) :: rc(1:3),r,sina,cosa,rot(1:3,1:3),rm(1:3),rcp(1:3),invrot(1:3,1:3),a(1:3),b(1:3),mag(1:3),la,gB(1:3)
  integer :: i,j,k,m
  ...
  ! compute the centre of the ring
    ...
    do i=1,cxz%nring
     ...
     rcp=matmul(rot,rc)
     call localB(mag,rcp,la,r,cxz%frames(j)%bondCurrent(i)*k_mju0/(4.0_k_pr*k_pi))
...
!> \f]
  ...
  subroutine localB(b,r,a,xr,pre)
  character(len=*), parameter :: myname="localB"
  real(k_pr), intent(inout) :: b(:)
  real(k_pr), intent(in) :: r(:),a,xr,pre
  real(k_pr):: d,h

    d=sqrt((r(1)-xr)*(r(1)-xr)+r(3)*r(3))
    h=((r(2)+a)/sqrt(d*d+(r(2)+a)*(r(2)+a))-(r(2)-a)/sqrt(d*d+(r(2)-a)*(r(2)-a)))/(d*d)
    b(2) = 0.0_k_pr
    b(1) = pre*h*r(3)
    b(3) =-pre*h*(r(1)-xr)
