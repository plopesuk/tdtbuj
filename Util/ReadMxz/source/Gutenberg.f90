...
contains
  ...
  subroutine PrintDipoles(mxz,unitout)
    ...
    write(unitout,'(a,i0,a)')"#to get the atom number for Z component substract ",6+2*mxz%natoms," from last number after ZCol"
    write(fmtContainer,'("(f16.8,1x,5f16.8,",i0,"f16.8,",i0,"f16.8,",i0,"f16.8)")') mxz%natoms,mxz%natoms,mxz%natoms
    do i=1,mxz%nframes
      write(unitout,trim(fmtContainer))mxz%frames(i)%timestamp,mxz%frames(i)%dipole,mxz%frames(i)%projDipole,mxz%frames(i)%tdx,mxz%frames(i)%tdy,mxz%frames(i)%tdz,&
