\chapter{Time Dependent Tight Binding+UJ}
\label{ch:one}
%
...
\subsection{Second order HKS functional}
\par{
...
\be
\begin{split}
 \Ket{n} & = \Ket{n^{\tm{in}}}+\Ket{\delta n} \\
 \rho^{\tm{in}}(\bm{r}) & =\sum_{n} f_n \Braket{\bm{r}|n^{\tm{in}}}\Braket{n^{\tm{in}}|\bm{r}}\\
 \rho(\bm{r}) & =\rho^{\tm{in}}(\bm{r}) +\delta \rho(\bm{r})=\sum_{n}f_n\Braket{\bm{r}|n}\Braket{n|\bm{r}}
\end{split}
\label{eq:defrhoin}
\ee
\par{Replacing $\rho$ in the HKS functional \marginpar{add HKS citation} it becomes}
 \be
E^{\tm{HKS}}[\rho]=T_s[\rho]+E_H\left[\rho^{\tm{in}}+\delta \rho\right]+E_{xc}\left[\rho^{\tm{in}}+\delta\rho\right]+E_{ZZ}+\int\rho V_{ext}\td\bm{r}
\label{eq:HKSb}
\ee
...
\end{equation}
\par{using $E_{H}\left[ \rho^{\tm{in}}\right]=\frac{e}{2}\int \rho^{\tm{in}}(\bm{r})V_{H}\left[ \rho^{\tm{in}}\right] \td{\bm{r}}$ we can evaluate the contributions of the Hartree and exchange-correlation up to the second order}
\begin{equation}
\begin{split}
...
\label{eq:ehexc}
\end{equation}
\par{using \gref{eq:defrhoin} and gathering all the second order terms we get}
 \begin{equation}
\begin{split}
...
\end{equation}
\par{which allows us to write the kinetic energy of the non-interacting electrons system as}
\begin{equation}
...
\end{equation}
\par{now we can write down the second order HKS functional by replacing eqs. \gref{eq:a} and \gref{eq:b} into eq. \gref{eq:HKSb}}
\begin{equation}
 \begin{split}
...
\label{eq:hks2}
\end{equation}
\par{This functional is very interesting. Up to this point has only one approximation in it, the truncation in $E_{xc}$ to the second order in $\delta \rho$. Looking for the stationary value of eq. \gref{eq:hks2} we get the Kohn-Sham equations}
\begin{equation}
...
\subsection{Semi-empirical tight binding}
\section{scrap}
...
$k$ must be also even.}
%
\subsection{+U for second order theory}
