    ...
    case (k_electrostaticsMultipoles)
      do i = 1, atomic%atoms%natoms
        if (GetLmax(atomic%atoms%sp(i), atomic%speciesBasis, atomic%species) > 0) then
          atomic%atoms%dx (i) = atomic%atoms%x(i) * atomic%atoms%chrg(i) + qlmr (i, 1, 1, gen, sol, atomic, tb, sol%density) * Sqrt &
         & (4*k_pi/3.0_k_pr)
          atomic%atoms%dy (i) = atomic%atoms%y(i) * atomic%atoms%chrg(i) + qlmr (i, 1,-1, gen, sol, atomic, tb, sol%density) * Sqrt &
         & (4*k_pi/3.0_k_pr)
          atomic%atoms%dz (i) = atomic%atoms%z(i) * atomic%atoms%chrg(i) + qlmr (i, 1, 0, gen, sol, atomic, tb, sol%density) * Sqrt &
         & (4*k_pi/3.0_k_pr)
        else
          atomic%atoms%dx (i) = atomic%atoms%x(i) * atomic%atoms%chrg(i)
          atomic%atoms%dy (i) = atomic%atoms%y(i) * atomic%atoms%chrg(i)
    ...
    if (gen%screened) then
      do k = 1, atomic%atoms%nscf
        ...
        do j = 1, atomic%atoms%natoms
          if (i /= j) then
            ...
            sol%potential (i) = sol%potential(i) + k_e2 * Charge (j, gen, atomic, sol) / &
           & (4.0_k_pr*k_pi*k_epsilon0*Sqrt(rij*rij+uv*uv))
          end if
        end do
      end do
    else
      do k = 1, atomic%atoms%nscf
        ...
        do j = 1, atomic%atoms%natoms
          if (i /= j) then
            rij = Distance (atomic%atoms, i, j)
            sol%potential (i) = sol%potential(i) + k_e2 * Charge (j, gen, atomic, sol) / (4.0_k_pr*k_pi*k_epsilon0*rij)
          end if
        end do
      end do
    end if
      ...
      do k = 1, atomic%atoms%nmoving
        ...
        do j = 1, atomic%atoms%natoms
          if (i /= j) then
            ...
            fact = - Charge (j, gen, atomic, sol) * k_e2 / (4.0_k_pr*k_pi*k_epsilon0) / (rij*rij+uv*uv) ** 1.5_k_pr
...
!
    ...
    do k1 = 1, atomic%species%norbs(atomic%atoms%sp(at))
      ...
      do k2 = 1, atomic%species%norbs(atomic%atoms%sp(at))
        ...
        sum = sum + density (aidx(atomic%atoms%orbs(at, k1), atomic%atoms%orbs(at, k2), n)) * tb%delta(atomic%atoms%sp(at))%d(l, &
       & l1, l2) * sol%rgc(idx(l1, m1), idx(l2, m2), idx(l, m))
      end do
    end do
    compQlmR = sum
!
...
!
    ...
    if (gen%hasElectricField) then
      if (l == 0) then
       ...
       & (atomic%atoms%x(i)*gen%E(1)+atomic%atoms%y(i)*gen%E(2)+atomic%atoms%z(i)*gen%E(3)) * k_e * 2.0_k_pr * Sqrt (k_pi) * etd &
       & (gen)
      else if (l == 1) then
...
!    |latex  \frac{X_{l^{\prime\prime}m^{\prime\prime}}(\hat{r_{12}})}{r_{12}^{l^{\prime\prime}+1}}\frac{(2l^{\prime\prime}+1)!!}{2l
!    |latex \end{equation}
!   with
...
!
    lpp = l + lp
    r = Sqrt (real(2*lpp+1, k_pr)/(4.0_k_pr*k_pi)) * fact3 (lpp, sol) / (2.0_k_pr*lpp+1.0_k_pr)
!     r=sqrt(x*x+y*y+z*z)**(lpp+1)
    sum = 0.0_k_pr
    do p = - lpp, lpp
      sum = sum + sol%rgc(idx(l, m), idx(lp, mp), idx(lpp, p)) * ir (idxy(lpp, p)) * r
    end do
...
!
    ...
    do l = 0, 2 * GetLmax (atomic%atoms%sp(at), atomic%speciesBasis, atomic%species)
      do m = - l, l
        sum = sum + sol%rgc(idx(lu, mu), idx(lv, mv), idx(l, m)) * tb%delta(atomic%atoms%sp(at))%d(l, lu, lv) * vlmr (at, l, m, &
       & gen, sol, atomic, tb, density)
      end do
    end do
...
!
    ...
    lp = li + 1
    do mp = - lp, lp
      sum = sum + sol%rgc(idx(1, p), idx(li, mi), idx(lp, mp)) * vlmr (at, lp, mp, gen, sol, atomic, tb, density)
    end do
    fip = sum
  end function fip
!
